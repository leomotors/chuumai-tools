...
>
  <div class="flex items-center justify-between gap-6">
    ...
    <!-- Mobile menu; visibility is CSS-only so the server-rendered markup is
         viewport-independent and does not flash on hydration. -->
    <Popover.Root bind:open={mobileOpen}>
      <Popover.Trigger
        class="flex size-9 cursor-pointer items-center justify-center rounded-full bg-gray-100 text-gray-700 transition-colors hover:bg-gray-200 md:hidden"
        aria-label="Menu"
      >
        <Menu class="size-5" />
      </Popover.Trigger>
    ...
    {#if user}
      <Popover.Root bind:open={userOpen}>
        <Popover.Trigger
          class="hidden cursor-pointer items-center gap-2 rounded-full bg-gray-100 py-1 pl-1 pr-3 transition-colors hover:bg-gray-200 md:flex"
        >
          {#if user.image}
            <img
              src={user.image}
    ...
    {:else}
      <Popover.Root>
        <Popover.Trigger
          class="hidden h-9 shrink-0 items-center justify-center gap-2 rounded-md bg-[#5865f2] px-4 py-2 text-sm font-medium text-white shadow-xs transition-all hover:bg-[#4752c4] md:inline-flex"
        >
          Sign In
        </Popover.Trigger>
        <Popover.Content
