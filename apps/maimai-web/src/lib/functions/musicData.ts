...
/**
 * Upsert a single chart's level & constant for a given version & chart type.
 * Used by web admins from the data page. Invalidates the table cache so the
...
export async function updateMusicLevel({
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [
        musicLevelTable.musicTitle,
        musicLevelTable.chartType,
