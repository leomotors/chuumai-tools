...
export async function getPlayCountSince(
  ...
  // Get the latest record for playCountCurrent and playCountTotal
  ...
  const dbPlayCountTotal = latestRecord[0].playCountTotal;
  const dbPlayCountCurrent = latestRecord[0].playCountCurrent;

  // If current play count is not provided, use the database value
  const currentCount = currentPlayCount ?? dbPlayCountTotal;

  // Calculate thisVersion with offset if currentPlayCount is provided
  const thisVersion =
    currentPlayCount !== undefined
      ? dbPlayCountCurrent + (currentPlayCount - dbPlayCountTotal)
      : dbPlayCountCurrent;
