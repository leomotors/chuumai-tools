<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    if (!selectedNextVersion || !selectedCurrentVersion) {
      submissionError = "Please select both versions";
      return;
    }
    ...
    try {
      ...
      link.href = url;
      link.download = `preview-next-${selectedNextVersion}-${Date.now()}.json`;
...
<div class="w-full max-w-2xl">
  ...
  <p class="text-gray-600 text-center mb-8">
    ...
    the next version. Songs from the current/next version will be treated as New
    songs, while others will be treated as Old songs.
  </p>
  ...
  <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="space-y-2">
        ...
        >
          Next Version (Chart Constant)
        </label>
        <select
          id="nextVersion"
          bind:value={selectedNextVersion}
      ...
      <div class="space-y-2">
        <label
          ...
          class="block text-sm font-medium text-gray-700"
        >
          Current Version (for New songs)
        </label>
        <select
          id="currentVersion"
          bind:value={selectedCurrentVersion}
          class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        >
          {#if enabledVersions.length === 0}
            <option value="" disabled>Error: No versions available</option>
    ...
    <!-- International Version Checkbox -->
    <div class="flex items-center space-x-2">
      <input
        type="checkbox"
        id="intlVersion"
        bind:checked={useIntlVersion}
        class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
      />
      <label for="intlVersion" class="text-sm text-gray-700">
        Use international release date to determine new songs
      </label>
    </div>
